...
int typeOfSentFrame(const QByteArray &frame)
{
    MassageHandler handler;
    int type = -1;
    QObject::connect(&handler, &MassageHandler::frameReady,
                     [&type](int msgType, const QByteArray &) {
                         type = msgType;
                     });
    handler.feed(frame);
    return type;
...
QJsonObject stationQueryPayload(const QJsonArray &stationRecords,
                                const QJsonArray &chargerRecords = {})
{
    QJsonArray stations;
    for (const QJsonValue &value : stationRecords) {
        if (!value.isObject()) {
        ...
        QJsonArray chargers;
        const QString name = station.value(QStringLiteral("stationName")).toString();
        for (const QJsonValue &chargerValue : chargerRecords) {
            if (chargerValue.isObject()
                && chargerValue.toObject().value(QStringLiteral("stationName")).toString()
                       == name) {
                chargers.append(chargerValue);
            }
        }
        station.insert(QStringLiteral("chargers"), chargers);
 ...
 * 覆盖：119/229 组合查询、字段映射、坏行跳过、关联/超时/取消/断连、
 * 错误映射与同类并发约束。
 */
...
private slots:
    ...
    void queryStationsUsesDedicatedCombinedProtocol();
...
void RealChargerServiceTests::queryStationsUsesDedicatedCombinedProtocol()
{
    ...
    QCOMPARE(typeOfSentFrame(m_transport->sentFrames.at(0)), STATION_QRY_REQ);
    QCOMPARE(payloadOfSentFrame(m_transport->sentFrames.at(0))
                 .value(QStringLiteral("requestId")).toString(),
             QStringLiteral("req-1"));

    feedResponse(m_transport, STATION_QRY_ACK,
                 stationQueryPayload({stationRecord(QStringLiteral("A站"),
                                                    113.9, 22.5, 150)}));
...
void RealChargerServiceTests::listAggregatesCountsFiltersAndPaginates()
{
    ...
    feedResponse(m_transport, STATION_QRY_ACK,
                 stationQueryPayload(
                     {stationRecord(QStringLiteral("A站"), 113.9, 22.5, 150),
                      stationRecord(QStringLiteral("A2站"), 113.95, 22.55, 160),
                      stationRecord(QStringLiteral("B站"), 114.0, 22.6, 180), dirty},
                     {chargerRecord(QStringLiteral("CP-1"), QStringLiteral("A站"),
                                    CHARGER_IDLE, true),
    ...
    feedResponse(m_transport, STATION_QRY_ACK,
                 stationQueryPayload({stationRecord(QStringLiteral("A站"), 113.9, 22.5, 150),
                                      stationRecord(QStringLiteral("A2站"), 113.95, 22.55, 160)}));
...
void RealChargerServiceTests::detailFailsEarlyWhenStationMissing()
{
    ...
    feedResponse(m_transport, STATION_QRY_ACK, stationQueryPayload({}));
    QCOMPARE(m_transport->sentFrames.size(), 1);
...
void RealChargerServiceTests::priceInYuanIsRoundedToCents()
{
    ...
    feedResponse(m_transport, STATION_QRY_ACK,
                 stationQueryPayload({yuanPriced}));
