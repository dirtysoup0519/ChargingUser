...
> **状态：已被服务端协议 v2.4 否决。** 最新服务端将 218 定义为
> `RESTART_ACK`，且未提供 117/118/219。本节仅保留历史决策，不再作为真实网络适配依据。
> 在服务端冻结普通用户专用资料接口前，真实适配器必须返回 `unsupported-protocol`，
> 不得借用 100/111；Mock 仍可用于业务层和流程层开发。
>
...
## 12. 合同变更记录 v1.2 —— 错误归属与会话隔离修订（2026-09-05）

> 变更动机：交付前代码审查发现 4 类会话一致性缺陷（失败请求卡死、无 ID 错误跨类型
> 误归属、换号重登会话污染、结果未知无法恢复）。全部为客户端侧修复，另附 1 项
> 服务端建议，不阻塞交付。

### 12.1 客户端侧修订（无需服务端改动，本节与前文冲突处以本节为准）

| # | 修订 | 取代/补充 |
|---|---|---|
| 1 | **失败应答归属规则**：无回显 requestId 的 3xx 错误，仅在全局恰好一个在途请求时才归属；0 个或 ≥2 个时丢弃，交由各请求自身超时兜底 | 取代 §11.2 R2“归属最近在途请求”（117 查询与 118 改昵称可并发，且存在“A 超时后重试 B”场景，归属最近请求会跨类型误归属） |
| 2 | **断线清理语义**：连接断开时在途查询/登录按可重试失败处理；在途改昵称（118）按**结果未知**处理，禁止自动重试 | 补充 §11 断连语义（服务端可能已处理 118，结果不可知） |
| 3 | **失败回执必须携带 requestId**：客户端所有失败路径（断线/发送失败）回填 requestId/operationId，保证上层释放对应在途请求 | 客户端内部实现约束 |
| 4 | **结果未知恢复规则**：117 资料查询成功返回即视为服务端状态已知，解除改昵称更新锁定；用户依据当前昵称决定是否重新提交 | 补充 §11.2 R5 的恢复闭环：M4 无需额外接口，查询成功即解锁 |
| 5 | **换号重登会话隔离**：已登录状态再次登录成功时提升会话世代并清空旧在途请求，旧账号迟到应答不得覆盖新会话 | 补充 §3 会话管理 |
| 6 | **成功应答校验收紧**：218 必须 `username` 非空且等于请求的 username、`phone` 非空；219 必须 `ok=true` 且 `nickname` 非空；不满足按应答损坏处理（218→可重试失败，219→结果未知） | 补充 §11.1 载荷定义 |

### 12.2 服务端建议（不阻塞交付，v2.2 协议建议项）

- **3xx 错误应答建议回显 `requestId`**（与 2xx 同样拷贝一个字段即可）。回显后
  客户端可精确归属错误，彻底消除 §12.1 第 1 条的丢弃兜底；未回显时客户端按
  第 1 条规则保守处理，功能不受影响。
