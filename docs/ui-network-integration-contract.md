...
ZXing 安装后，正式入口已支持从相册读取二维码。识别结果只接受 JSON 中的
`chargerCode`、URL 查询参数 `chargerCode` 或严格字符集的纯电桩编号；随后必须与
当前由服务端返回的站点详情匹配，并检查 `canCharge`，通过后才进入充电确认 Binder。
二维码原文不会写入日志。摄像头实时采集仍待 Qt Multimedia 环境就绪后实现。

### 9.9 阶段 D 订单交互补齐

订单页返回、刷新和订单动作已接入真实对象图：充电中订单进入会话页，待结算订单查询
权威详情后由既有路由进入结算页，非充电订单显示明确的未接入提示。虚拟机已安装
Qt Multimedia 5.15.3 开发包，但当前不存在 `/dev/video*` 摄像头设备，因此实时摄像头
扫码只允许做编译验证；图片二维码路径作为本轮可执行的扫码联调入口。

## 10. 后续代码实现规划

### 10.1 地图真实底图与诊断完善

- 准备不提交到 Git 的 `config/tencent-map.local.json` 或 `TENCENT_MAP_KEY`；
- 验证 QtWebEngineProcess、HTTPS、WebGL 和腾讯 Key 白名单；
- 首页与详情页共用同一 Key、区域和降级策略；
- 增加地图加载超时、JS 错误、渲染进程退出和重试测试；
- 地图失败时仍保证站点列表、电桩详情和充电入口可用。

### 10.2 扫码服务层与摄像头

- 新增 `modules/scanner` 领域接口和 `ScanResult` DTO；
- Qt Multimedia 负责摄像头枚举、权限、采集帧和手电筒；
- ZXing 负责图片/视频帧二维码解码；
- Binder 校验二维码格式、长度、站点/电桩匹配和重复处理；
- 通过 `IChargerService` 查询服务端权威电桩状态，成功后复用充电确认流程；
- 无摄像头环境必须保留图片导入路径。

### 10.3 订单和个人中心完善

- 扩展订单查询条件，支持历史订单、预约订单和分页；
- 增加订单详情页及预约订单动作；
- 将常用充电站改为真实数据源；
- 将帮助与反馈、关于页面从当前提示替换为正式页面；
- 为所有返回路径补充页面栈测试，避免回到错误页面。

### 10.4 端到端联调与质量门槛

- 覆盖启动、登录、站点、地图、选桩、扫码、启动、会话、停止、结算、支付、钱包、
  订单和退出登录；
- 每个写操作核对服务端日志、数据库记录和客户端状态；
- 超时、断线、重复点击和迟到应答必须保持结果未知纪律；
- 每阶段必须通过自动化测试、Linux 全量构建和人工联调记录。

## 11. 当前版本完整联调流程

### 11.1 联调前准备

服务端数据库至少准备：一个测试用户、一个在线空闲可启动电桩、一个正在充电订单、
一个待结算订单、一个可预约电桩，以及可记录充值流水的钱包账户。

客户端构建与启动：

```bash
cd ~/WorkSpace/QtProjects/APP/ChargingUser
qmake CONFIG+=real_network
make -j4
export CHARGER_SERVER_HOST=10.194.99.223
...
启动日志应出现服务器地址、`Network state: connected` 和腾讯地图 Key 状态，且不得
出现读取 TMP 业务数据的日志。

### 11.2 已完成能力测试顺序

1. 登录/自动注册：新用户测试阶段密码使用手机号，登录后检查服务端唯一用户记录。
2. 资料修改：修改昵称、保存，重新进入或登录确认值来自服务端。
3. 站点列表：验证站点、电桩、搜索、清除、重试、排序和详情入口均使用服务端数据。
4. 站点详情：验证电桩状态、选择态、充电入口和不可用原因。
5. 地图：验证有效 Key 下首页/详情底图、标记和点击；失败时验证降级列表与重试。
6. 钱包：验证余额、流水、充值金额字段 `amount`/`amountCents`、`216` 回执和不重复入账。
7. 订单：进入我的订单，验证活动订单、查看充电、去结算、刷新和返回路径。
8. 预约：选择可预约电桩、确认预约、押金、倒计时、取消和余额流水。
9. 充电确认：验证钱包余额、桩状态和服务端错误；不可用电桩不得本地放行。
10. 扫码图片：分别测试合法二维码、不存在桩、不可用桩、非法内容和坏图片；只有服务端
    匹配且可充电的桩才能进入确认页。
11. 活动订单恢复：准备充电中/待结算订单后重新登录，分别验证会话页/结算页恢复。
12. 停止与结算：停止充电、生成结算单、支付，核对订单、余额和流水一致。
13. 退出与返回：从首页、钱包、订单、详情、扫码、会话、结算页面返回并退出登录，
    确认重新登录不残留上一用户数据。

### 11.3 每项测试记录

每条用例记录客户端时间和页面、请求/响应结果、服务端日志关键行、数据库变更前后值、
是否重复写入、错误码及截图/终端日志。手机号、密码、余额和订单号外发时必须脱敏。
