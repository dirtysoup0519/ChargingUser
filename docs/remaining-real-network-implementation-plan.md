...
主“充电”页内嵌的会话组件现已转发扫码意图，不再出现底部按钮无响应。图片识别仅
提取 `chargerCode`，随后发送 `119 STATION_QRY_REQ`，由 `229 STATION_QRY_ACK`
解析真实所属站点及电桩状态，再进入充电确认页；该链路不依赖本地 TMP、首页缓存或
用户预先打开某个站点详情。扫码页返回会按入口回到主充电页或独立会话页。

### 正式入口接线说明（当前阶段）

`main.cpp` 现在同时承接主窗口内嵌会话页和二级会话页的刷新、订单切换、停止及扫码意图，
并兼容站点详情页的旧版导航/充电信号。页面跳转由入口统一管理，不在页面内直接创建顶层窗口。

以下协议能力因本地仅有旧版服务端，暂不猜测字段或强行开启：

- `108/208` 启动充电的幂等与结果查询能力；当前正式确认页仍显示服务端能力限制。
- 预约扣款、退款及预约订单在 `214` 中的订单类型/字段映射。
- 钱包充值流水是否作为订单返回，以及历史订单分页字段。

这些字段需要服务端源码或抓包样例确认后，再补充对应网络适配器和测试，不使用 TMP 冒充真实结果。
